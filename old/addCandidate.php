						...
						<div class="m-portlet">
							...
							<div class="m-portlet__body">	
								<div class="m-section__content">
									...
									<form class="m-form m-form--fit m-form--label-align-right" action="t2.php" method="post">
										<div class="m-portlet__body">
											
											<div class="form-group m-form__group">
												<label for="Username">Select Candidate</label>
											  <select class="form-control m-input m-input--solid" name="user_id" id="user_id">
												  <option value="">Please Choose..</option>
												  <?
														$sql_events = mysqli_query($connection, "select * from profile,ipta where profile.ipta_id=ipta.ipta_id and  phase='3' ") or die (mysql_error());
															while ($row = mysqli_fetch_array($sql_events)) {
																$user_id = $row["user_id"];
																$name = $row["name"];
																$ipta = $row["ipta"];
												  ?>
												 <option value="<? echo $user_id ?> "><? echo $name ?> (<? echo $ipta ?>) </option>
												  <? } ?>
													
											  </select>
												<span class="m-form__help">Candidate form Phase III</span>
											</div>
											
											<div class="form-group m-form__group">
												<label for="Authentication">Select Assessment</label>
												<select class="form-control m-input m-input--solid" name="prog_id" id="prog_id">
													<option value="">Please Choose..</option>
													<?
														$sql_events = mysqli_query($connection, "select * from training") or die (mysql_error());
															while ($row = mysqli_fetch_array($sql_events)) {
																$prog_id = $row["prog_id"];
																$prog_name = $row["prog_name"];
																$prog_date = $row["prog_date"];
												  ?>
												 <option value="<? echo $prog_id ?> "><? echo $prog_name ?> on <? echo $prog_date ?> </option>
												  <? } ?>
